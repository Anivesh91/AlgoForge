 ...
 */
async function generateAndValidateProblem({ userId, prompt, difficulty = 'Medium', topic = 'General DSA' }) {
  const MAX_RETRIES = 3;
  let attempt = 0;
  let lastFeedback = '';
  let lastError = null;
  ...
  while (attempt < MAX_RETRIES) {
    attempt++;
    console.log(`[ProblemGenerator] Generation attempt ${attempt}/${MAX_RETRIES} for prompt: "${prompt}"`);
